'use client'
import { Card, CardContent, CardHeader, Container, Grid2, IconButton, Typography, Stack, Box } from "@mui/material";
        ...
        <Container>
            ...
            <Grid2 container direction={'row'} spacing={3} sx={{width: '100%'}}>
                {loadedPlaylists.map((playlist) =>           
                    ...
                    <Grid2 item size={{lg: 3, md: 4, sm: 6}} key={playlist.id}> {/* {{ edit_2: Added key prop for list items }} */}
                        <Card> 
                            <CardContent> 
                            <Box>                    
                                <Image src={playlist.videos[playlist.videos.length - 1].thumbnails.maxres.url}
                                    alt={playlist.title}
                                    layout="responsive"
